...
"call s:initVarialbe("g:NERDTreeMapCloseOrJumpParent", "<left>")
...
"        let @h=@h."\" ". g:NERDTreeMapCloseOrJumpParent .": close node or go to parent\n"
...
"    exec "nnoremap <silent> <buffer> ". g:NERDTreeMapCloseOrJumpParent ." :call <SID>closeOrJumpToParent()<cr>"
...
" FUNCTION: s:closeOrJumpToParent() {{{2
" close the current directory, or jump to parent if already closed
function! s:closeOrJumpToParent()
    let treenode = s:TreeFileNode.GetSelected()
    if treenode ==# {}
        return
    endif
    let closed = treenode.close()
    if closed ==# 1
        call s:jumpToParent()
    endif
endfunction
